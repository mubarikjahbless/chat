...
export class AuthService {
    constructor(@InjectModel(UserModel.name) private readonly userModel: Model<UserModel>, private readonly apiResponseService: ApiResponseService,) { }
    ...
    public async login(user: User) {
        ...
        if (result) {
            ...
            if (isMatch) {
                ...
                await result.save();
                return this.apiResponseService.successResponseWithData('successful', loginPayload({ id: result._id.toString(), name: result.name } as User))
            }
        }
